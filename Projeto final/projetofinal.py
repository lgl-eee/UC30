a = 1
filmesdisponiveis = ["teste","Vingadores: Ultimato", "O Rei Leão(Audiodescrição)", "Frozen II", "Toy Story 4", "Coringa", "Star Wars: A Ascensão Skywalker(Audiodescrição)", "Homem-Aranha: Longe de Casa", "Capitã Marvel(Audiodescrição)", "Aladdin", "It", "Dumbo", "Malévola: Dona do Mal(CC)", "O Gato de Botas 2: O Último Pedido", "A Bela e a Fera", "Mulan", "Shazam!", "Dora e a Cidade Perdida(Audiodescrição)", "Jumanji(CC)", "Ad Astra: Rumo às Estrelas(Audiodescrição)", "Aves de Rapina: Arlequina e sua Emancipação Fantabulosa(Audiodescrição)"]
filmesalugados = []
dano = "(danificado)"

cadastro = input("Digite seu nome de usuário: ")
...
while a >= 1:
    if loginUsuario == cadastro and loginSenha == senha or a == 1:
        a = 1
        print("\n1. Alugar um filme")
        print("2. comprar um filme")
        print("3. devolver um filme")
        print("4. Vender um filme")
        print("5. Ver os filmes no catálogo")
        print("6. Sair")
        ...
        if escolha == 1:
            ...
            if filme3 in filmesalugados:
                estadofilme = input("O filme está em bom estado? (sim/nao): ")
                if estadofilme == "sim": 
                    ...
                    filmesdisponiveis.append(filme3 + dano)
                    filmesalugados.remove(filme3)
            else:
            ...
            if filme4 in filmesdisponiveis or filme4 in filmesalugados:
                ...
                estadofilme2 = input("O filme está em bom estado? sim/nao: ")
                if estadofilme2 == "nao":
                    print("O preço do filme será reduzido devido ao estado danificado.")
                    filmesdisponiveis.append(filme4 + dano)
                else:
            ...
            a = 2
            loginUsuario = input("\nDigite seu nome de usuário para fazer login: ")
            loginSenha = input("Digite sua senha para fazer login: ")
